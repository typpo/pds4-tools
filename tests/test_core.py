...
class TestLabel(PDS4ToolsTestCase):
    ...
    def test_getters(self):
        ...
        assert isinstance(one_liner1.getroot(), ET.Element)
    ...
    def test_find(self):
        ...
        assert isinstance(title2, ET.Element)
    ...
    def test_findall(self):
        ...
        assert isinstance(title2[0], ET.Element)
    ...
    def test_to_string(self):
        ...
        namespace1 = ' xmlns:fake_prefix="http://pds.nasa.gov/pds4/fake_prefix/v1"'
        py26_namespace1 = namespace1 if is_py26 else ''
        py27plus_namespace1 = namespace1 if (not is_py26) else ''
        ...
        expected_string1 = (
            ...
            '    </Target_Identification>'
        ).format(py27plus_namespace1, py26_namespace1)

        assert string1 == expected_string1

        # Make Python 2.6 adjustments, which includes prefix in more local spot than later Python versions
        namespace2 = ' xmlns="http://pds.nasa.gov/pds4/pds/v1" xmlns:fake_prefix="http://pds.nasa.gov/pds4/fake_prefix/v1"'
        py26_namespace2 = namespace2 if is_py26 else ''
        py27plus_namespace2 = namespace2 if (not is_py26) else ''
        ...
        expected_string2 = (
            ...
            '  <fake_prefix:label_keyword{1}>test element</fake_prefix:label_keyword>'          '\n'
            '</Target_Identification>'
        ).format(py27plus_namespace2, py26_namespace2)
...
class TestTableDataTypes(PDS4ToolsTestCase):
    ...
    def test_overflow(self):
        ...
        # Test ASCII_Numeric_Base16 overflow to 'object' dtype (bigger than int64)
    ...
    def test_scaling(self):
        ...
        _check_array_equal(table['Integer Scaling/Offset 1'], scaled_integers1, 'int64')
        _check_array_equal(table['Integer Scaling/Offset 2'], scaled_integers2, 'float64')

        # Test Float Scaling/Offset
        _check_array_equal(table['Float Scaling/Offset'], [-3.2e+48, 3.2e+48, 1234.0], 'float64')
...
def _check_array_equal(unknown_array, known_array, known_typecode):
    ...
    # Check that all other (non-float and non-complex) values are equal
        ...
        assert np.array_equal(unknown_array, np.asarray(known_array, dtype='object'))
